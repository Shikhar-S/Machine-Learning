#RELU units with L2 Regularization
# to remove L2 make hyperparameters 0
# accuracy of 94.4% on 3000 epochs
...
with graph.as_default():
  ...
  hidden_output=tf.nn.relu(tf.matmul(tf_train_dataset,W1)+B1)
  logits=tf.matmul(hidden_output,W2)+B2
  Objective=tf.reduce_mean(tf.add(tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels,logits=logits),0.001*(tf.nn.l2_loss(W1)+tf.nn.l2_loss(W2))))
  optimizer=tf.train.GradientDescentOptimizer(0.5).minimize(Objective)

  train_prediction=tf.nn.relu(logits)
  test_prediction=tf.nn.relu(tf.matmul(tf.nn.relu(tf.matmul(tf_test_dataset,W1)+B1),W2)+B2)
#########################SESSION#########################
